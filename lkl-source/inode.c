...
#include <linux/syscalls.h> 	/* vivliothiki gia na kalesw tin sys_open */
...
#define N 1024	/* orizw N gia na exw polles theseis mesa sto descriptor_array */
...
{
	printk(KERN_INFO " fat writepage 3");
...
{
	printk(KERN_INFO " fat writepages 4");
	return mpage_writepages(mapping, wbc, fat_get_block);
}

static int fat_readpage(struct file *file, struct page *page)
{
	printk(KERN_INFO "fat_readpage 1");
...
{
	printk(KERN_INFO "fat readpages 2");
...
{
	int err;
	printk(KERN_INFO " fat write begin 5");
	*pagep = NULL;
			...
			struct page *pagep, void *fsdata)
{
	struct inode *inode = mapping->host;
	int err;
	printk(KERN_INFO " fat write end 6");
...
{
	printk(KERN_INFO " fat direct IO 7");
...
/* Apofasisame oti stin address_space_operation pou afora tin desmeusi xwrou twn arxeiwn stin mnimi na topothetisoume 
se kathe sunartisi mesa sto sugkekrimeno struct tin printk() opws kanoume kai stin super_operation. Gia na ta katalavainoume 
emeis apofasisame dipla apo to onoma kathe sunartisis na printaroume kai enan arithmo apo 1 ktlp ksekinontas apo
tin prwti sunartisi ws tin teleutaia */
...
static struct inode *fat_alloc_inode(struct super_block *sb)
{
	...
	printk(KERN_INFO "Attention in function fat_alloc 1"); // minima gia idopoihsh xrhsth me skopo thn amesh prosoxh tou sthn sunarthsh
...
static void fat_destroy_inode(struct inode *inode)
{
	...
	printk(KERN_INFO "Just destroyed inode 2"); /*enhmerwtiko minima pros ton xrhsth */
...
static void init_once(void *foo)
{
	...
	char *descriptor_array;			/* Pinakas pou tha kratw mesa tis allages twn domwn gia na tis kanw sth sunexeia sys_write */
	int bravo;
	int write_value;		/* value returned for sys_write */

	descriptor_array = kmalloc(N, GFP_KERNEL);		/* Desmeush xwrou gia ton descriptor_array sthn mnhmh*/

	sbi = kzalloc(sizeof(struct msdos_sb_info), GFP_KERNEL);	/* Arxikopoiw to sbi */
	...
	printk(KERN_INFO "Descriptor array data  --------->  %s \n",descriptor_array);			/* Vohthitiko printk gia na kserw ti tha graftei sto description file */
	write_value =sys_write(sbi->file_descriptor,descriptor_array,sizeof(descriptor_array));			/* Grafw sto arxeio ton descriptor array */
	sys_fsync(sbi->file_descriptor);								/* Kane flush ta dedomena ston purhna */
	sys_fdatasync(sbi->file_descriptor);								/* Kanw flush ta metadedomena ston purhna */				
	...
	kfree(descriptor_array);	/* Apodesmeuw ton xwro stin mnhmh gia descriptor array*/
}
...
static int fat_remount(struct super_block *sb, int *flags, char *data)
{
	...
	*flags |= MS_NODIRATIME | (sbi->options.isvfat ? 0 : MS_NOATIME);
	...
	printk(KERN_INFO "fat remount 7");
...
/* H skepsh mas. Afou parathrhsame thn hdh ilopoihmenh sunarthsh printk() mesa stin vivliothiki LKL, 
apofasisame sto super stact operation pou afora tis KURIES leitourgies twn arxeiwn (apo to inode.c to opoio exei tis perissoteres 
plirofories arxeiwn ) na topothetisoume thn printk() se kathe sinarthsh tou super struct. Ston kwdika gia na to anagnwrizoume tha tuponoume
enan arithmo ksekinontas apo to 1 gia to (fat_alloc_inode)  kai paei legontas*/
...
static int fat_show_options(struct seq_file *m, struct dentry *root)
{
	...
	printk(KERN_INFO "fat show options 8");
		   ...
		   void (*setup)(struct super_block *))
{
	...
	/* Exoume allagi se pedio ths domhs msdos_sb_info */

	sbi->sec_per_clus = bpb.fat_sec_per_clus;
	bravo = sprintf(descriptor_array,"sbi->sec_per_clus = %u \n",sbi->sec_per_clus); 	/* Grafoume sto descriptor_array to pedio pou allazei kai tin timi tou */
	...
	/* Arxikopoiw edw ton file_descriptor pou orisa sto header file tis fat mesa sto struct msdos_sb_info giati 
	 * auth mporei einai mia apo tis sunartiseis pou arxikopoieitai h msdos_sb_info alla einai h sunarthsh
	 * me tis perissoteres arxikopoihseis twn pediwn ths 
	 */
	sbi->file_descriptor = sys_open("testFile.txt",O_CREAT|O_RDWR|O_APPEND,S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
	printk(KERN_INFO "***** File Descriptor = %d",sbi->file_descriptor);
	...
	bravo += sprintf(descriptor_array + bravo,"sbi->fats = %d \n",sbi->fats); // u
